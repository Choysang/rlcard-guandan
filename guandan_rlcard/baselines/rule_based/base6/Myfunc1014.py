...
class Myfunc1014(object):
    def firstcard(self,hd, cur):
        ...
        for i in hdcopy:
            ...
            if i.find(cur) == 1 and i[0] == 'H':
                dict['numh'].append(i)
        # #_debug_print(len(self.getcard(hdcopy, dict['numh'])['Straight'][1]))
        while len(self.getcard(hdcopy, dict['numh'])['Straight']) > 0:
            # #_debug_print("长度",len(getcard(hdcopy))['Straight'][1::2])
            ##_debug_print("getcard的顺子", self.getcard(hdcopy, dict['numh'])['Straight'])
        ...
        ##_debug_print("这是先手配的连对",dict['ThreePair'])
        if len(dict['ThreePair']) > 0:
            for i in dict['ThreePair'][1::2]:
                for j in i:
                    if poke.index(j[1])<=10:
                        ##_debug_print("这是j",j,j[1],poke.index(j[1]))
                        hdcopy.remove(j)
       # #_debug_print("这是hdcopy", hdcopy)
        #_debug_print("这是dict",dict)
        dict['Single']=[]
        dict['Pair']=[]
        dict['Trips']=[]
        #_debug_print("这是先手配的牌",dict)
        return (hdcopy, dict)

    def convert_action_list_to_dict(self, action_list):
        ...
        for action in action_list:
            type_, rank, cards = action

            # 忽略不在字典中的类型（例如 PASS）
            if type_ not in result:
                continue

            result[type_].append(rank)
            result[type_].append(cards)
                       ...
                       # #_debug_print("这是eb", eb, num)
                        for i in eb:
                            if single.count(i) == 1:
                                ...
                                # #_debug_print(dict['Pair'][dict['Pair'].index(i)+1][0])
                                num -= 1
                               # #_debug_print("这是eb", eb, num)
                            elif trip.count(i) == 1:
        ...
        for i in poke:
            if trip.count(i) == 1:
                ...
                if len(ez) >= 2:
                    ##_debug_print(ez)
                    for i in ez:
                        ##_debug_print(dict['Trips'][dict['Trips'].index(i) + 1][0])
                        ##_debug_print(dict['Trips'][dict['Trips'].index(i) + 1][1])
                        cldui.append(dict['Trips'][dict['Trips'].index(i) + 1][0])
                        cldui.append(dict['Trips'][dict['Trips'].index(i) + 1][1])
    ...
    def getcolor(self, dict, cur):
        #_debug_print("getcolordict",dict)
        index = []
        len1=0
        min = 99
        ls = []
        ...
        if len(dict['Straight']):
            for i in dict['Straight'][1::2]:
                ...
                ls.append([i])
                # #_debug_print(ls[0])
                #_debug_print("先手出了顺子",ls[0])
            #return ls[0][0]
            return ls[0]
        if len(dict['ThreePair']) and poker.index(dict['ThreePair'][0])<=10:
           ##_debug_print("这是连对",dict['ThreePair'])
            for i in dict['ThreePair'][1::2]:
                ##_debug_print(dict['ThreePair'][1::2])
                ls.append([i])
               # #_debug_print(ls[0])
                #_debug_print("先手出了连对",ls[0])
            #return ls[0][0]
            return ls[0]
        if len(dict['TwoTrips']) and poker.index(dict['TwoTrips'][0])<=8:
            ##_debug_print("这是钢板",dict['TwoTrips'])
            for i in dict['TwoTrips'][1::2]:
               # #_debug_print(dict['TwoTrips'][1::2])
                ls.append([i])
               # #_debug_print(ls[0])
                #_debug_print("先手出了钢板",ls[0])
            #return ls[0][0]
            return ls[0]
    ...
    def getindex(self, actionlist, color):
        # 如果 color 是嵌套列表，则取出第一项
        if isinstance(color, list) and len(color) > 0 and isinstance(color[0], list):
            color = color[0]
        #_debug_print("这是color",color)
...
# gp=msg['greaterPos']
# act=data['actionList']
# #_debug_print(msg['curPos'])
# #_debug_print(lasthand.firstcard(hd,'2')[0])
# #_debug_print(lasthand.firstcard(hd,'2')[1])
# #_debug_print("二手配完牌",lasthand.lgetcard(lasthand.firstcard(hd,'2')[0],lasthand.firstcard(hd,'2')[1]))
# # #_debug_print("寻找索引",lasthand.getindex(act,lasthand.getcolor(lasthand.lgetcard(lasthand.firstcard(hd,'2')[0],lasthand.firstcard(hd,'2')[1]),'2')))
# #_debug_print("这是手牌",lasthand.lgetcard(lasthand.firstcard(hd,'2')[0],lasthand.firstcard(hd,'2')[1]))
# #_debug_print("index",lasthand.getindex(act,lasthand.getcolor(lasthand.lgetcard(lasthand.firstcard(hd,msg['curRank'])[0],lasthand.firstcard(hd,msg['curRank'])[1]),'2')))
# #_debug_print("这是要出的牌",lasthand.getcolor(lasthand.lgetcard(lasthand.firstcard(hd,msg['curRank'])[0],lasthand.firstcard(hd,msg['curRank'])[1]),'2'))
# #_debug_print("拿到索引",lasthand.getindex(act,['S3', 'C3']))
