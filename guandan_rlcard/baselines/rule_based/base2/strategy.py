...
class Strategy(object):
    ...
    def SetBeginning(self, myPos, handCard):
        ...
        self.origin_rests = copy.deepcopy(self.restCardsCount)
        # _debug_print('hands：' + str(handCard))
        # _debug_print('origin：' + str(self.origin_rests))
    ...
    def SetRole(self, handValue, handActions, curRank):
        ...
        for action in handActions:
            ...
            if action['type'] == 'Bomb' or action['type'] == 'StraightFlush':
                ...
                countBigs += 1
        # _debug_print(handValue, handActions, curRank)
        # _debug_print('bombs:',countBombs,'bigs',countBigs,'smalls:',countSmalls)
        self.role = ""
    ...
    def UpdatePlay(self, msg, num_cards_left, curPos, curAction, greaterPos, greaterAction):  # '3', ['Pair','3','['S3','H3']]
        # _debug_print("UpdatePlay:", curPos, curAction, greaterPos, greaterAction)
        # if (curAction and curAction[0] != 'PASS'):
            # self.restHandsCount[curPos] -= len(curAction[2])
        self.restHandsCount = num_cards_left
        ...
        # if (curAction == [] or curAction[0] == 'PASS'):  # update recordPlayerActions
        ...
        # 更新玩家反应记录
        self.recordPlayerActions = msg['reactions']
            
        # 计算剩余手牌池 
        # if (curPos != self.myPos and curAction and curAction[0] != 'PASS'):
            # for card in curAction[2]:
            #     self.restCardsCount[card[1]] -= 1
            
        # 计算剩余手牌池
        self.restCardsCount = copy.deepcopy(self.origin_rests)
        
        for id, sd in msg['history'].items():
            if id == str(self.myPos): continue
            # _debug_print(id, sd['send'])
            for card in sd['send']:
                self.restCardsCount[card[1]] -= 1
        # _debug_print(self.myPos, msg['history'][str(self.myPos)]['send'])
        # _debug_print(self.restCardsCount, end="\n\n")
    ...
    def UpdateRVByRestHandsCount(self):
        ...
        if (5 in C_oppo):
            self.freeActionRV['Pair'] += 1
            self.freeActionRV['ThreeWithTwo'] -= 0.5
        # _debug_print(oppo1, oppo2, self.greaterPos)
